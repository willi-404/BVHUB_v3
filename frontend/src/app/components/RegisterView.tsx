...
const required: Field[] = ["displayName", "firstName", "lastName", "email", "street", "houseNumber", "postalCode", "city", "birthDate"];
...
export default function RegisterView() {
  ...
  const [values, setValues] = useState<RegistrationInput>({ displayName: "", firstName: "", lastName: "", email: "", street: "", houseNumber: "", postalCode: "", city: "", birthDate: "", phone: "", contactInfo: "" });
  ...
  const input = (field: Field, label: string, type = "text", optional = false) => {
    ...
    return <div className="flex flex-col gap-1.5"><label htmlFor={`register-${field}`} className="text-sm font-medium text-foreground">{label}{optional ? ` (${t("register.optional")})` : ""}</label><Input id={`register-${field}`} ref={(el) => { refs.current[field] = el; }} type={type} value={value} onChange={(e) => update(field, e.target.value)} aria-invalid={Boolean(error)} aria-describedby={error ? `error-${field}` : undefined} className={`h-11 ${error ? "border-red-500 ring-1 ring-red-200" : submitted ? "border-emerald-500 ring-1 ring-emerald-100" : ""}`} />{error && <p id={`error-${field}`} className="text-xs text-red-600">{error}</p>}</div>;
  };
  return <div className="min-h-full flex flex-col items-center justify-center px-4 py-8 bg-[var(--background)]"><div className="w-full min-w-0 max-w-2xl"><div className="flex min-w-0 items-center gap-3 mb-6"><img src={logoSrc} alt={t("brand.logoAlt")} className="h-12 w-12 shrink-0 object-contain" /><div className="min-w-0"><h1 className="page-title">{t("register.title")}</h1><p className="break-words text-sm text-[var(--muted-foreground)]">{t("register.subtitle")}</p></div></div><form onSubmit={submit} noValidate className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-5 md:p-7 flex flex-col gap-4">{input("displayName", t("register.displayName"))}<div className="grid md:grid-cols-2 gap-4">{input("firstName", t("register.firstName"))}{input("lastName", t("register.lastName"))}</div>{input("email", t("register.email"), "email")}<div className="grid md:grid-cols-2 gap-4">{input("street", t("register.street"))}{input("houseNumber", t("register.houseNumber"))}{input("postalCode", t("register.postalCode"))}{input("city", t("register.city"))}</div>{input("birthDate", t("register.birthDate"), "date")}<div className="grid md:grid-cols-2 gap-4">{input("phone", t("register.phone"), "tel", true)}{input("contactInfo", t("register.contactInfo"), "text", true)}</div>{serverError && <p role="alert" className="break-words text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">{serverError}</p>}<Button type="submit" size="lg" disabled={loading}>{loading ? `${t("register.submit")} …` : t("register.submit")}</Button><p className="text-sm text-center text-[var(--muted-foreground)]"><Link to="/login" className="text-[var(--primary)] hover:underline">{t("register.backToLogin")}</Link></p></form></div></div>;
}
