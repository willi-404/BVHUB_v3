...
export default function RegisterView() {
  ...
  return <div className="min-h-full flex flex-col items-center justify-center px-4 py-8 bg-[var(--background)]"><div className="w-full max-w-2xl"><div className="flex items-center gap-3 mb-6"><img src={logoSrc} alt={t("brand.logoAlt")} className="h-12 w-12 object-contain" /><div><h1 className="text-xl font-700">{t("register.title")}</h1><p className="text-sm text-[var(--muted-foreground)]">{t("register.subtitle")}</p></div></div><form onSubmit={submit} noValidate className="bg-[var(--card)] border border-[var(--border)] rounded-[var(--radius)] p-5 md:p-7 flex flex-col gap-4">{input("displayName", t("register.displayName"))}<div className="grid md:grid-cols-2 gap-4">{input("firstName", t("register.firstName"))}{input("lastName", t("register.lastName"))}</div>{input("email", t("register.email"), "email")}<div className="grid md:grid-cols-2 gap-4">{input("street", t("register.street"))}{input("houseNumber", t("register.houseNumber"))}{input("postalCode", t("register.postalCode"))}{input("city", t("register.city"))}</div>{input("birthDate", t("register.birthDate"), "date")}<div className="grid md:grid-cols-2 gap-4">{input("phone", t("register.phone"), "tel", true)}{input("contactInfo", t("register.contactInfo"), "text", true)}</div>{serverError && <p role="alert" className="text-sm text-red-600 bg-red-50 border border-red-200 rounded p-3">{serverError}</p>}<Button type="submit" size="lg" disabled={loading}>{loading ? `${t("register.submit")} …` : t("register.submit")}</Button><p className="text-sm text-center text-[var(--muted-foreground)]"><Link to="/login" className="text-[var(--primary)] hover:underline">{t("register.backToLogin")}</Link></p></form></div></div>;
}
