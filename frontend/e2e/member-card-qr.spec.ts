...
async function mockMemberSession(page: Page) {
  ...
  }, { initialToken: sessionToken, record: user })
  await page.route("http://127.0.0.1:18099/api/**", async (route: Route) => {
    const request = route.request()
    const path = new URL(request.url()).pathname
    if (path === "/api/collections/users/auth-refresh") return route.fulfill({ json: { token: sessionToken, record: user } })
    if (path === "/api/bvhub/me/profile") return route.fulfill({ json: { user, profile: null, groups: [{ membershipId: "membership", id: "member-er", name: "Member ER", active: true, created: user.created, updated: user.updated }] } })
    ...
    if (path === "/api/bvhub/member-card/verify") return route.fulfill({ json: { valid: true, member: { id: user.id, displayName: user.displayName, groups: ["Member ER"] }, tokenExpiresAt: "2099-01-01T00:02:00.000Z", verifiedAt: "2026-09-14T20:00:00.000Z" } })
